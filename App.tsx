        ...
        <div className="grid grid-cols-6 gap-2 p-2 bg-slate-200 rounded-lg mt-4">
            {keys.map(key => (
                <button
                    ...
                    className={`h-14 rounded-lg text-xl font-bold transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 active:translate-y-px
                        ${key === 'OK' ? 'col-span-6 bg-sky-500 text-white hover:bg-sky-600 shadow-md hover:shadow-lg' : ''}
                        ${key === '⌫' ? 'bg-slate-400 text-white hover:bg-slate-500 shadow-sm hover:shadow-md' : ''}
                        ${!['OK', '⌫'].includes(key) ? 'bg-white text-slate-700 hover:bg-slate-50 shadow-sm hover:shadow-md' : ''}
                    `}
                >
                    {key}
                </button>
