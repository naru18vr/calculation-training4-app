...
const Keypad = ({ onKeyPress }: { onKeyPress: (key: string) => void }) => {
    ...
    return (
        <div className="grid grid-cols-6 gap-2 p-2 bg-slate-200 rounded-lg mt-4">
            {keys.map(key => (
                <button
                    key={key}
                    onClick={() => onKeyPress(key)}
                    className={`h-14 rounded-lg text-xl font-bold transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 shadow-md active:shadow-inner active:translate-y-px
                        ${key === 'OK' ? 'col-span-6 bg-sky-500 text-white hover:bg-sky-600' : ''}
                        ${key === '⌫' ? 'bg-rose-500 text-white hover:bg-rose-600' : ''}
                        ${!['OK', '⌫'].includes(key) ? 'bg-white text-slate-700 hover:bg-slate-50' : ''}
                    `}
                >
                    {key}
                </button>
    ...
    return (
        <div className="p-4 sm:p-6 text-center">
            ...
            <div className="space-y-4 sm:space-y-0 sm:flex sm:space-x-4 justify-center">
                <button onClick={onRetry} className="w-full sm:w-auto px-8 py-3 bg-sky-500 text-white font-bold rounded-lg shadow-md hover:shadow-lg hover:bg-sky-600 transition-all transform hover:-translate-y-0.5 active:translate-y-0">もう一度挑戦</button>
                <button onClick={onBackToTop} className="w-full sm:w-auto px-8 py-3 bg-slate-600 text-white font-bold rounded-lg shadow-md hover:shadow-lg hover:bg-slate-700 transition-all transform hover:-translate-y-0.5 active:translate-y-0">トップに戻る</button>
            </div>
        </div>
    );
};
