  ...
  switch (topic.id) {
    ...
    case 'g4_division': {
        if (difficulty === '基礎') { // 余りなし
            ...
            text = `${dividend} ÷ ${divisor} の商と余りを求めなさい。(例: 15あまり5)`;
    ...
    case 'g4_unit_conversion': {
        if (difficulty === '基礎') {
            ...
            if (toCm) {
                ...
                text = `${cm}cm は何m何cmですか？(例: 2m50cm)`;
                answer = `${Math.floor(cm/100)}m${cm%100}cm`;
                explanation = `100cmが1mです。${cm}cmは${answer}です。`;
            }
        } else if (difficulty === '標準') {
            ...
            text = `${val}${conv.to} は何${conv.from}何${conv.to}ですか？(例: 1kg500g)`;
            answer = `${Math.floor(val/conv.factor)}${conv.from}${val%conv.factor}${conv.to}`;
            explanation = `1${conv.from}は${conv.factor}${conv.to}です。答えは${answer}です。`;
        } else { // 発展
