  ...
  switch (topic.id) {
    ...
    case 'm3_quadratic_equations_factorization': { // 二次方程式（因数分解）
        if (difficulty === '基礎') {
            ...
            text = `二次方程式 x^2 ${bStr} ${cStr} = 0 を解きなさい。(x= の形で, 複数の場合はカンマ区切りで)`;
            ...
            text = `x^2 ${-root >= 0 ? `+ ${-root}` : `- ${root}`}x = 0 を解きなさい。(x= の形で, 複数の場合はカンマ区切りで)`;
            ...
            text = `${x2Coeff}x^2 ${xStr} ${constStr} = 0 を解きなさい。(x= の形で, 複数の場合はカンマ区切りで)`;
    ...
    case 'm3_quadratic_equations_formula': { // 二次方程式（解の公式）
        ...
        text = `二次方程式 ${aStr}${bStr}${cStr} = 0 を解の公式を使って解きなさい。(x= の形で, 複数の場合はカンマ区切りで)`;
